...
describe("Ranking Service (Híbrido - Caixa Preta e Branca)", () => {
	beforeEach(() => {
		dbMock.mockReset();
	});

	describe("buscarRanking", () => {
		test("deve retornar Top 20 e Posição do Usuário perfeitamente (Fluxo Feliz / Caixa Preta)", async () => {
			const top20Mock = [
				{
					id: 1,
					name: "Alice",
		...
		test("deve aplicar máscara de anonimato para usuários que não são o logado (Caixa Branca)", async () => {
			const top20Mock = [
				{
					id: 1,
					name: "Carlos",
		...
		test("deve lidar com o desvio interno de usuário sem pontuação executando uma terceira query (Caixa Branca)", async () => {
			const idUsuarioMock = 99;
			
			// 1ª Chamada: Top 20 (apenas outros usuários)
			const top20Mock = [
				{ id: 1, name: "Alice", pointsTotalEarned: 100, rankingAnonymous: false, insigniaMaisRecente: null }
			];
			...
			const usuarioFallbackMock = [
				{ name: "Bob Sem Pontos", pointsTotalEarned: 0, insigniaMaisRecente: null }
			];
		...
		test("deve lidar com usuário inexistente no fallback da terceira query (Caixa Branca)", async () => {
			dbMock
		...
		test("deve lançar ErroRankingIndisponivel se a query de Top 20 falhar (Caixa Branca / Exceções)", async () => {
			dbMock.mockRejectedValueOnce(new Error("Falha no banco"));

			await expect(buscarRanking(1)).rejects.toBeInstanceOf(ErroRankingIndisponivel);
			
			// Verifica que parou na primeira chamada
			expect(dbMock).toHaveBeenCalledTimes(1);
		});

		test("deve lançar ErroRankingIndisponivel se a query de posição falhar (Caixa Branca / Exceções)", async () => {
			dbMock
		...
		test("deve lançar ErroAnonimato se houver falha durante o loop de mapeamento (Caixa Branca / Exceções)", async () => {
			...
			dbMock
				.mockResolvedValueOnce(top20Mock)
				.mockResolvedValueOnce([{ posicao: 1, name: "A", pointsTotalEarned: 10 }]);
