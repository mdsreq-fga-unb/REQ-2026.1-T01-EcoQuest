...
describe("Reward Service — resgatarRecompensaInsignia", () => {
  // Extraindo os módulos que não haviam sido cobertos
  const {
  ...
  test("resgata recompensa atrelada a insígnia com sucesso", async () => {
    dbMock
      ...
      .define(() => Promise.resolve([recompensaDB({ id: 1 })])) // 3: Busca da Recompensa (OK)
      .define(() => Promise.resolve([])) // 4: Já resgatado antes? (Não)
      .define(() =>
        Promise.resolve([
          // 5: CTE de Geração e Débito atômico
          {
            redemptionId: 99,
            code: "INS-A1B2C3D4",
            custoPontos: 0, // Custo deve ser 0!
            criadoEm: "2026-07-01T10:00:00.000Z",
            expiraEm: null,
            saldoApos: 1000, // O saldo permanece intacto
          },
        ]),
      );

    const resultado = await resgatarRecompensaInsignia(1, 10, 1);

    expect(resultado.id).toBe(99);
    expect(resultado.custoPontos).toBe(0); // Resgate vinculado a Insígnia deve ser gratuito
  ...
  test("lança ErroRecompensaJaResgatada se usuário tentar resgatar o prêmio duas vezes", async () => {
    dbMock
      ...
      .define(() => Promise.resolve([{ 1: 1 }]))
      .define(() => Promise.resolve([recompensaDB()]))
      .define(() => Promise.resolve([{ 1: 1 }])); // Tabela reward_redemption retorna registro existente!
  ...
  test("encapsula erros de banco como ErroFalhaGeracaoCupom se a CTE falhar ou quebrar", async () => {
    dbMock
      ...
      .define(() => Promise.resolve([]))
      .define(() =>
        Promise.reject(new Error("Conexão perdida no meio da CTE")),
      );
